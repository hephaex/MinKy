...
const RelatedDocsList = ({ documentId, limit = 5 }) => {
  ...
  if (error) {
    return (
      <aside className="kb-related" aria-label="관련 문서">
        <h3 className="kb-related-title">관련 문서</h3>
        <ErrorMessage error={error} title="관련 문서를 불러올 수 없습니다" onRetry={execute} />
      </aside>
    );
  }
          ...
          <li key={doc.id || i} className="kb-related-item">
            <Link to={`/documents/${doc.id}`} className="kb-related-link">
              <div className="kb-related-meta">
                ...
                <span className="kb-related-doc-title">{doc.title || '제목 없음'}</span>
              </div>

              {doc.summary && <p className="kb-related-summary">{doc.summary}</p>}

              <div className="kb-related-footer">
                {doc.score != null && (
                  <span className="kb-related-score">{Math.round(doc.score * 100)}% 유사</span>
                )}
