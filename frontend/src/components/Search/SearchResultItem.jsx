...
const SearchResultItem = ({ result, query = '' }) => {
  const { id, title, excerpt, score, tags = [], updated_at, source } = result;
        ...
        <div className="kb-result-footer">
          {tags.length > 0 && (
            <div className="kb-result-tags" aria-label="태그">
              ...
              {tags.length > 3 && (
                <span className="kb-result-tag kb-result-tag--overflow">+{tags.length - 3}</span>
              )}
            </div>
          )}
          <div className="kb-result-meta">
            {source && <span className="kb-result-source">{source}</span>}
            {updated_at && <span className="kb-result-date">{formatDate(updated_at)}</span>}
          </div>
        </div>
      </Link>
    </article>
