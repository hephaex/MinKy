...
def map_document_fields(row: dict[str, Any], default_user_id: int) -> dict[str, Any]:
    ...
    return {
        ...
        # dict.get(key, default) only substitutes when key is MISSING, not when value is None.
        # Use explicit None-check to coerce DB NULLs to safe defaults for NOT NULL columns.
        ...
        "is_published": row.get("is_published") if row.get("is_published") is not None else False,
