...
describe('ChatHistory', () => {
  it('renders empty state with New Chat button', () => {
    render(<ChatHistory sessions={[]} onSelect={jest.fn()} onNew={jest.fn()} />);
    expect(screen.getByText('No conversations yet')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /new chat/i })).toBeInTheDocument();
  });

  it('renders session list items', () => {
    render(<ChatHistory sessions={sessions} onSelect={jest.fn()} onNew={jest.fn()} />);
  ...
  it('calls onSelect when a session is clicked', () => {
    ...
    render(<ChatHistory sessions={sessions} onSelect={onSelect} onNew={jest.fn()} />);
  ...
  it('calls onNew when New button is clicked', () => {
    const onNew = jest.fn();
    render(<ChatHistory sessions={sessions} onSelect={jest.fn()} onNew={onNew} />);
  ...
  it('marks active session', () => {
    render(
      <ChatHistory sessions={sessions} activeId="s1" onSelect={jest.fn()} onNew={jest.fn()} />
    );
  ...
  it('calls onDelete when delete button is clicked', () => {
    ...
    render(
      <ChatHistory sessions={sessions} onSelect={jest.fn()} onNew={jest.fn()} onDelete={onDelete} />
    );
