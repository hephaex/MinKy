    ...
    const loadDocuments = async () => {
      try {
        ...
        const response = await api.get(
          `/documents/by-date?date_key=${encodeURIComponent(dateKey)}&page=1&per_page=50`
        );
  ...
  if (error) {
    return (
      ...
      >
        ...
        >
          <h3 style={{ margin: '0 0 16px 0', fontSize: '24px' }}>오류가 발생했습니다</h3>
  ...
  return (
    ...
    >
      ...
      {documents.length === 0 ? (
        ...
        >
          <h3 style={{ margin: '0 0 16px 0', fontSize: '24px', color: '#999' }}>문서가 없습니다</h3>
          <p style={{ margin: '0', fontSize: '16px' }}>이 기간에 작성된 문서가 없습니다.</p>
        </div>
      ) : (
        <div
          style={{
        ...
        >
          {documents.map((doc) => (
            ...
            >
              ...
              >
                {doc.author && (
                  <span style={{ fontSize: '14px', color: '#666' }}>작성자: {doc.author}</span>
                )}
